...
import { catalogCategories, catalogEntries } from "@/lib/catalog";
        ...
        {options.map((opt) => (
          <button
            ...
            className={
              ...
              (lang === opt.key ? "bg-primary text-primary-foreground" : "text-foreground/70 hover:bg-secondary")
            }
          >
            {opt.label}
          </button>
    ...
    <header className="sticky top-0 z-40 border-b border-border bg-background/95 backdrop-blur supports-[backdrop-filter]:bg-background/85">
      <div className="mx-auto flex max-w-6xl flex-wrap items-center justify-between gap-x-6 gap-y-3 px-4 py-3">
        <a href="#top" className="font-serif text-lg font-bold tracking-tight">
          Mascardo Legacy
          <span className="ml-2 hidden text-xs font-normal uppercase tracking-[0.25em] text-muted-foreground md:inline">1871–1932</span>
        </a>
        <nav className="order-3 flex w-full flex-wrap items-center gap-x-5 gap-y-1 sm:order-none sm:w-auto">
          {links.map(({ href, label, icon: Icon }) => (
            <a key={href} href={href} className="flex items-center gap-1.5 font-serif text-sm text-foreground/75 underline-offset-4 transition-colors hover:text-foreground hover:underline">
  ...
  return (
    <section id="top" className="relative overflow-hidden border-b border-border">
      <div className="mx-auto grid max-w-6xl gap-10 px-4 pb-14 pt-12 sm:pt-16 lg:grid-cols-[1fr_1.15fr] lg:items-center lg:gap-14">
        <motion.figure initial={{ opacity: 0, scale: 0.97 }} animate={{ opacity: 1, scale: 1 }} transition={{ duration: 0.7, ease: "easeOut" }} className="vintage-frame mx-auto w-full max-w-xs p-3 sm:max-w-sm">
          <img src={t.gallery.items[0].src} alt={t.gallery.items[0].caption} loading="eager" className="aspect-[4/5] w-full object-cover sepia-photo grayscale-[35%]" />
          <figcaption className="mt-3 border-t border-border pt-2 text-center font-serif text-xs italic text-muted-foreground">{t.hero.photoCaption}</figcaption>
        </motion.figure>
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.7, delay: 0.1, ease: "easeOut" }}>
          <p className="smallcaps-label text-muted-foreground">{t.hero.kicker}</p>
          <h1 className="mt-4 font-serif text-4xl font-black leading-tight tracking-tight sm:text-5xl lg:text-6xl">{t.hero.name}</h1>
          <p className="mt-4 max-w-xl font-serif text-lg italic text-primary/90 sm:text-xl">{t.hero.subtitle}</p>
  ...
  return (
    <motion.section {...fadeUp} className="mx-auto max-w-4xl px-4 pt-12">
      <div className="vintage-frame p-6 sm:p-8">
        <h2 className="text-center font-serif text-xl font-bold tracking-wide">{t.quickFacts.title}</h2>
        <Ornament />
        ...
        <div className="mt-10 space-y-10">
          {chapters.map((ch) => (
            <motion.article key={ch.title} {...fadeUp} className="relative pl-6 sm:pl-8">
              <span aria-hidden className="absolute left-0 top-1 h-[calc(100%-0.5rem)] w-px bg-gradient-to-b from-transparent via-primary/50 to-transparent" />
      ...
      <div className="mx-auto max-w-4xl px-4 py-16">
        ...
        <ol className="relative mt-10 space-y-0">
          {t.timeline.items.map((item, i) => (
            <motion.li key={item.year + i} initial={{ opacity: 0, x: -16 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true, margin: "-60px" }} transition={{ duration: 0.45, ease: "easeOut" }} className="relative grid grid-cols-[5.5rem_1fr] gap-4 border-b border-dashed border-border py-4 last:border-0 sm:grid-cols-[7rem_1fr]">
              <span className="font-serif text-sm font-bold uppercase tracking-wider text-primary">{item.year}</span>
...
function CatalogSection() {
  ...
  const results = useMemo(() => {
    ...
    return catalogEntries.filter((entry) => {
      ...
      const haystack = [entry.title, entry.years ?? "", entry.summary.en, entry.summary.fil, ...entry.tags].join(" ").toLowerCase();
      return terms.every((term) => haystack.includes(term));
    });
  }, [query, active]);
  const chips: { key: ActiveCategory; label: string }[] = [
    { key: "all", label: t.catalog.categories.all },
    ...catalogCategories.map((c) => ({ key: c as ActiveCategory, label: t.catalog.categories[c] })),
  ];
  return (
      ...
      <div className="mx-auto max-w-5xl px-4 pt-20">
        ...
        <motion.div {...fadeUp} className="mt-10">
          <div className="relative mx-auto max-w-xl">
            ...
            <input type="search" value={query} onChange={(e) => setQuery(e.target.value)} placeholder={t.catalog.searchPlaceholder} aria-label={t.catalog.searchPlaceholder} className="w-full rounded-sm border border-border bg-card py-2.5 pl-9 pr-9 font-serif text-sm text-foreground placeholder:italic placeholder:text-muted-foreground/70 focus:border-ring focus:outline-none focus:ring-1 focus:ring-ring" />
            {query ? <button type="button" onClick={() => setQuery("")} aria-label={t.catalog.clearSearch} className="absolute right-2.5 top-1/2 -translate-y-1/2 text-muted-foreground transition-colors hover:text-foreground"><X className="size-4" aria-hidden /></button> : null}
          </div>
          <div className="mt-5 flex flex-wrap items-center justify-center gap-2">
            {chips.map((chip) => (
              <button key={chip.key} type="button" onClick={() => setActive(chip.key)} aria-pressed={active === chip.key} className={"rounded-full border px-4 py-1.5 font-serif text-xs tracking-wide transition-colors sm:text-sm " + (active === chip.key ? "border-primary bg-primary text-primary-foreground" : "border-border bg-transparent text-muted-foreground hover:border-foreground/40 hover:text-foreground")}>{chip.label}</button>
            ))}
          </div>
          <p className="mt-4 text-center text-xs uppercase tracking-widest text-muted-foreground">{t.catalog.entryCount(results.length)}</p>
        </motion.div>
        {results.length > 0 ? (
          <div className="mt-6 grid gap-x-8 gap-y-2 sm:grid-cols-2">
            {results.map((entry) => (
              <motion.article key={entry.id} layout initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.3 }} className="group border-b border-dashed border-border py-4">
                <div className="flex items-baseline justify-between gap-3">
                  <h3 className="font-serif text-lg font-semibold leading-snug">{entry.title}</h3>
                  {entry.years ? <span className="shrink-0 font-serif text-xs italic text-muted-foreground">{entry.years}</span> : null}
                </div>
                <p className="smallcaps-label mt-0.5 text-primary/80">{t.catalog.categories[entry.category]}</p>
                <p className="mt-2 text-sm leading-relaxed text-foreground/80">{entry.summary[lang]}</p>
          ...
          <motion.div {...fadeUp} className="mx-auto mt-8 max-w-md py-12 text-center">
            ...
            <p className="mt-2 text-sm leading-relaxed text-muted-foreground">{t.catalog.noResultsBody}</p>
            <button type="button" onClick={() => { setQuery(""); setActive("all"); }} className="mt-5 rounded-sm border border-border bg-card px-4 py-2 font-serif text-sm text-foreground transition-colors hover:bg-secondary">{t.catalog.clearSearch}</button>
          </motion.div>
        )}
      </div>
    </section>
  ...
  return (
    <section id="movie" className="scroll-mt-20">
      <div className="mx-auto max-w-5xl px-4 pt-20">
        ...
        <motion.h3 {...fadeUp} className="mt-12 text-center font-serif text-xl font-bold tracking-wide">{t.movie.scenesTitle}</motion.h3>
        <Ornament />
          ...
          {t.movie.clips.map((clip) => (
            <motion.figure key={clip.id} {...fadeUp} className="vintage-frame flex flex-col p-3">
              <div className="relative aspect-video w-full overflow-hidden border border-border bg-black/90">
                <iframe src={`https://www.youtube-nocookie.com/embed/${clip.id}?rel=0`} title={clip.situation} allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" referrerPolicy="strict-origin-when-cross-origin" allowFullScreen loading="lazy" className="absolute inset-0 h-full w-full" />
              </div>
              <figcaption className="flex grow flex-col pt-4">
                <p className="smallcaps-label text-primary">{clip.source}</p>
                <blockquote className="mt-2 font-serif text-lg font-semibold italic leading-snug">"{clip.situation}"</blockquote>
                <p className="mt-2 grow text-sm leading-relaxed text-muted-foreground">{clip.caption}</p>
              </figcaption>
            </motion.figure>
          ))}
        </div>
        <motion.div {...fadeUp} className="vintage-frame mx-auto mt-12 max-w-3xl p-6 sm:p-8">
          ...
          <p className="mt-6 border-t border-dashed border-border pt-4 text-center text-xs italic text-muted-foreground">{t.movie.disclaimer}</p>
        </motion.div>
      </div>
    </section>
  );
        ...
        <div className="mt-10 grid gap-8 sm:grid-cols-2 lg:grid-cols-4">
          {t.gallery.items.map((item, i) => (
            <motion.figure key={i} {...fadeUp} className="vintage-frame p-3">
              <img src={item.src} alt={item.caption} loading="lazy" className="aspect-square w-full border border-border object-cover sepia-photo" />
              <figcaption className="pt-3 text-center font-serif text-xs italic leading-snug text-muted-foreground">{item.caption}</figcaption>
            </motion.figure>
          ))}
        </div>
      </div>
    ...
    <footer className="py-14">
      <motion.div {...fadeUp} className="mx-auto max-w-2xl px-4 text-center">
        ...
        <ul className="mt-5 space-y-2 text-sm text-muted-foreground">
          {t.footer.sources.map((s, i) => <li key={i}>{s}</li>)}
        </ul>
